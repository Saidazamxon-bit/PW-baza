...
const BoshSahifa = () => {
  const [mijozlar, setMijozlar] = useState(() => JSON.parse(localStorage.getItem("mijozlar")) || []);
  ...
  useEffect(() => { localStorage.setItem("mijozlar", JSON.stringify(mijozlar)) }, [mijozlar]);
  ...
  const oChirish = (id) => { if(window.confirm("Rostdan o‘chirmoqchimisiz?")) setMijozlar(mijozlar.filter(m=>m.id!==id)); };
  const tahrirlash = (m) => { setEditMijoz(m); setShowForm(true); };
  const copyKodlar = () => { navigator.clipboard.writeText(JSON.stringify(mijozlar, null, 2)); alert("✅ Mijoz kodlari nusxa olindi!"); };

  const filterlangan = mijozlar.filter(m => [m.ism, m.familya, m.id, m.telefon].some(x => x && x.toLowerCase().includes(qidiruv.toLowerCase())));
  ...
  return (
    <div style={{ marginTop:20, padding:"0 10px" }}>
      ...
      {showKodlar && <textarea readOnly style={{width:"100%", height:300, marginTop:10, padding:10, borderRadius:6, border:"1px solid #ccc", backgroundColor:"#f9f9f9", fontFamily:"monospace", fontSize:13}} value={JSON.stringify(mijozlar,null,2)} />}
